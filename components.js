...
class DashboardSidebar extends HTMLElement {
  connectedCallback(){const item=(target,label,icon,extra='')=>`<a class="nav-link ${extra}" href="#${target}" data-view-link="${target}">${iconMarkup(icon)}<span>${label}</span></a>`;this.innerHTML=`<a class="brand" href="#dashboard" data-view-link="dashboard"><svg aria-hidden="true" viewBox="0 0 32 36" fill="none"><path d="M5 29C2 12 11 5 28 4c-1 17-8 26-23 25Z" stroke="white" stroke-width="5"/><path d="m3 34 19-22" stroke="white" stroke-width="4"/></svg>Ideal Trends</a><nav class="sidebar-nav" aria-label="Navegação principal"><p class="nav-group">Resultados</p>${item('dashboard','Dashboards','home','active')}${item('performance','Performance','chart')}${item('business-plan','Business Plan','activity')}${item('insights','Insights','trend')}<p class="nav-group">Conhecimento</p>${item('knowledge','Playbook','book')}${item('knowledge','Produtos','layers')}${item('knowledge','Mercado','trend')}${item('knowledge','FAQ','activity')}${item('knowledge','Templates','book')}${item('knowledge','Onboarding','users')}${item('knowledge','TIME','users','nested-title')}${item('knowledge','Organograma','users','nested')}${item('knowledge','LINKS','link','nested-title')}${item('knowledge','Salesforce','link','nested')}${item('knowledge','Discador','phone','nested')}${item('knowledge','Ferramentas','gear','nested')}<p class="nav-group">Operação</p>${item('operation','Políticas','book')}${item('operation','Processos','gear')}${item('operation','Rituais','calendar')}</nav><svg class="sidebar-wave" viewBox="0 0 190 190" preserveAspectRatio="none" aria-hidden="true"><path d="M-20 37C60 36 67 182 211 150" fill="none" stroke="#112c55" stroke-width="38"/><path d="M-10 104C95 26 97 170 215 143" fill="none" stroke="#102645" stroke-width="35"/></svg>`;}
}
