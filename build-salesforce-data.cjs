...
function build(payload){
  ...
  const expected=[];
  for(let year=payload.startYear;year<=payload.endYear;year++)for(const metric of ['appointments','connections','sales','pipeline']){
  ...
  const ensure=(date,id)=>{
    ...
    if(!rows.has(key))rows.set(key,{date,month:date.slice(0,7),supervisorId:id,supervisorName:supervisors.get(id),revenue:0,appointments:0,connections:0,sales:0,pipeline:0});
    return rows.get(key);
  };
  for(const batch of payload.batches){
    ...
    for(const record of (Array.isArray(batch.records)?batch.records:batch.records?[batch.records]:[])){
      ...
      count+=record.n;
      if(batch.metric==='sales'||batch.metric==='pipeline'){
  ...
  for(const day of rows.values()){
    ...
    if(!monthly.has(key))monthly.set(key,{month:day.month,supervisorId:day.supervisorId,supervisorName:day.supervisorName,revenue:0,appointments:0,connections:0,sales:0,pipeline:0});
    const row=monthly.get(key);
    for(const metric of ['revenue','appointments','connections','sales','pipeline'])row[metric]=row[metric]===null||day[metric]===null?null:row[metric]+day[metric];
  }
...
if(require.main===module){
  try{
    ...
    console.log(JSON.stringify({status:'loaded',extractedAt:data.extractedAt,groups:data.rows.length,months:data.range,currentMonth:Object.fromEntries(['appointments','connections','sales','revenue','ticket','pipeline'].map(k=>[k,valueFor(data,payload.today.slice(0,7),k)])),warnings:data.warnings},null,2));
