...
class CRMTeam extends HTMLElement{
  set data({supabase,month}){
    ...
    const prospectingMembers=supabase.members.filter(member=>member.role==='hunter'&&member.status==='active'&&!hiddenSupervisorIds.has(member.supervisorId)&&(member.supervisorId===prospectingCloserId||member.supervisorId===matheusId||!member.supervisorId||(!knownSupervisorIds.has(member.supervisorId)&&member.supervisorId!=='unassigned-hunter')));
    if((prospectingMembers.length||supabase.members.some(member=>member.role==='hunter'&&member.status==='active'&&member.supervisorId==='unassigned-hunter'))&&!supervisors.some(supervisor=>supervisor.id===prospectingCloserId))supervisors.push({id:prospectingCloserId,name:'Prospecção Closer'});
    const memberIds=id=>id===prospectingCloserId?prospectingMembers.concat(supabase.members.filter(member=>member.role==='hunter'&&member.status==='active'&&member.supervisorId==='unassigned-hunter')).map(member=>member.id):supabase.members.filter(member=>member.role==='hunter'&&member.status==='active'&&member.supervisorId===id).map(member=>member.id);
