...
function bpProjection(row,elapsed,total){
  if(row.actual===null||elapsed<=0)return null;
  // Taxas e ticket são indicadores de qualidade, não acumulados. Mantemos o
  // nível atual como projeção; para volumes e valores, projetamos a cadência
  // entregue por dia útil até o encerramento do mês.
  if(row.type==='rate'||row.fixedPlan)return row.actual;
  const remaining=Math.max(0,total-elapsed);
  return row.actual+(row.actual/elapsed)*remaining;
}
...
function bpRender(){const month=bpMonthInput.value;if(!month)return;const targetDate=month===dataset.asOfDate.slice(0,7)?dataset.asOfDate:`${month}-${String(new Date(Date.UTC(+month.slice(0,4),+month.slice(5,7),0)).getUTCDate()).padStart(2,'0')}`;const elapsed=DashboardData.businessDayOrdinal(targetDate,dataset);const total=DashboardData.businessDaysInMonth(month,dataset);const ratio=Math.min(1,elapsed/total);const rows=bpDefinition(month).map(row=>({...row,planned:row.type==='rate'||row.fixedPlan?row.plan:row.plan===null?null:row.plan*ratio}));
 bpStatus.textContent=`Planejamento até o ${elapsed}º dia útil de ${total} (${new Intl.DateTimeFormat('pt-BR',{timeZone:'UTC'}).format(new Date(`${targetDate}T00:00:00Z`))}). A projeção usa a produtividade por dia útil já entregue e a aplica aos ${Math.max(0,total-elapsed)} dias úteis restantes.`;
 bpTable.innerHTML=`<div class="table-scroll"><table><caption>Meta do mês versus ritmo até o mesmo dia útil. A projeção soma o realizado ao ritmo médio diário multiplicado pelos dias úteis restantes. Taxas e TM mantêm o nível atual, pois não são acumulados. Ligações mostra CNPJs trabalhados e, abaixo, o total de ligações. “—” indica dado ainda indisponível.</caption><thead><tr><th>Indicador</th><th>Plano mês</th><th>Planejado até hoje</th><th>Realizado</th><th>Gap</th><th>Diagnóstico</th><th>Projeção</th></tr></thead><tbody>${rows.map(row=>{const comparable=row.comparisonActual??row.actual,gap=comparable===null||row.planned===null?null:comparable-row.planned,projection=bpProjection(row,elapsed,total);const actualHtml=row.detail?`<strong>${bpFormat(row.actual,row.type)}</strong><span class="bp-cell-detail">${escapeHTML(row.detail)}</span>`:bpFormat(row.actual,row.type);return `<tr><th scope="row">${escapeHTML(row.label)}</th><td>${bpFormat(row.plan,row.type)}</td><td>${bpFormat(row.planned,row.type)}</td><td>${actualHtml}</td><td class="${gap===null?'':gap<0?'gap-negative':'gap-positive'}">${bpGap(gap,row.type)}</td><td class="diagnosis">${escapeHTML(bpDiagnosis({...row,actual:comparable},row.planned))}</td><td class="bp-projection">${bpFormat(projection,row.type)}</td></tr>`;}).join('')}</tbody></table></div>`;
