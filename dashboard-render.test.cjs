...
assert.equal(roots['#macro-metrics'].children.length,0);
assert.equal(roots['#history-metrics'].children.length,8);
assert.match(roots['crm-team'].innerHTML,/Vendas \(R\$\)/);
assert.match(roots['crm-closer-team'].innerHTML,/Conversão/);
